...
insert into
    "COACH" (user_id, experience_years, description)
values
    (
        (
            select
...
set
    ...
    status = '上課中'
where
    id = 'a1e20a1f-ddf5-4709-9ce7-b1ba56258ac0';

-- 5-6. 查詢：計算用戶王小明的購買堂數，顯示須包含以下欄位： user_id , total。 (需使用到 SUM 函式與 Group By)
select
    user_id,
    count(course_id) total
from
    "COURSE_BOOKING" cb
    left join "USER" u on user_id = u.id
where
...
from
    "COACH" c
    inner join "COACH_LINK_SKILL" cls ON id = cls.coach_id
